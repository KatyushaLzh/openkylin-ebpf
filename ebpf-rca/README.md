...
| 目录 | 说明 |
|---|---|
| `bpf/cpu.bpf.c` | 场景① CPU 采集探针（sched tracepoint，内核态聚合） |
| `internal/collector` | 加载/挂载 eBPF，按窗口读取并差分 |
| `internal/detector` | 时序异常判定（持续高占用规则，可扩展 EWMA/3-sigma） |
| `internal/rca` | 确定性根因推断 + 证据链生成 |
| `internal/schema` | 统一结构化输出 schema（7 字段，含证据链） |
| `internal/output` | JSON / YAML / Markdown 渲染 |
| `cmd/ebpf-rca` | CLI 入口 |
| `scripts` | 一键部署 / 复现场景脚本 |
...
测试与复现细则见 [docs/testing.md](docs/testing.md)；本地自动化 E2E 见
[docs/local-testing.md](docs/local-testing.md)；容器部署见 [docs/docker.md](docs/docker.md)；
实测排查见 [docs/troubleshooting.md](docs/troubleshooting.md)。
...
- [x] 容器化部署（Docker / Podman，见 [docs/docker.md](docs/docker.md)）
- [ ] 多架构实测 / RISC-V 适配
