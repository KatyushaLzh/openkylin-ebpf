...
产物写入 `outputs/{env,repro,accuracy,bench,validation}/`。`run_all_repro.sh` 和 `bench_overhead.sh` 使用内置纯 workload，不复用 `scripts/repro_*.sh`，避免重复启动工具。当前 x86_64 单轮五类复现全部 PASS；2026-07-08 多轮 oracle 评测 27/27 有效，诊断准确率 100.0%，端到端通过率 100.0%，详见 `outputs/accuracy/accuracy.md`；benchmark 结果见 `outputs/bench/bench.md`。
...
- `raw_syscalls` 是最高开销场景，生产建议配合 `--target-pid`；mem/lock/syscall 的 target 会按 root pid 动态跟踪子进程。
