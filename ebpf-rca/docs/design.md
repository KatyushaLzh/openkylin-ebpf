...
sched_switch / sched_wakeup (tracepoint)
        ...
        ▼
   stats hash map  ──(用户态按 interval 读取 + 差分)──▶  Sample{cpu_util, ctx/min, runq_wait}
        ▼
...
| 指标 | 来源 | 计算 |
|---|---|---|
| `cpu_util` | `run_ns` 差分 | `Δrun_ns / interval_ns`，1.0 ≈ 占满一核 |
...
在 `block_rq_issue` 记录请求起点（按 `dev+sector` 索引）并使队列深度 +1；在
`block_rq_complete` 匹配起点算单次时延，按设备累计次数/总时延/最大时延/字节数，
并落入 log2(ns) 直方图，队列深度 -1。用户态按窗口差分并从直方图估算 P99。
...
危害最大。`mm_vmscan_direct_reclaim_begin/end` 按进程(tgid)累计直接回收次数与耗时，
以 `current` 归因；`mm_vmscan_kswapd_wake` 统计后台回收唤醒。这些 tracepoint 跨架构、
...
根因判定：肇事进程有直接回收 → "持续申请内存触发直接回收，回收压力上升"；
仅 major fault 升高 → "工作集超出物理内存"；都不明显 → "系统级低内存/OOM 风险"。
**这是一个混合采集的刻意设计**：eBPF 抓因果事件(谁触发回收)，/proc 补系统上下文。

## 4b. 场景④：锁竞争（off-CPU + 唤醒链）
...
**waker→wakee 唤醒链**，定位疑似持锁方。

| 证据 | 来源 | 含义 |
|---|---|---|
| `offcpu_ratio` | `offcpu_ns` 差分 / 窗口 | 阻塞型 off-CPU 占墙钟比例 |
| `max_offcpu_ms` | `max_offcpu_ns` | 单次最长阻塞 |
...
根因判定：阻塞栈含 `futex/mutex/rwsem/down_/__lock` 等符号 → 判为"锁竞争"，否则判为
"长时间阻塞等待"（疑似 I/O / 同步等待）。符号化在用户态完成，内核态零额外上送。
...
用户态按架构(amd64 专用表 / asm-generic 表)解析。

| 证据 | 来源 | 含义 |
|---|---|---|
| `syscall` | nr→名解析 | 热点系统调用 |
| `calls_per_sec` | `count` 差分 | 调用频次 |
| `avg_lat_us` / `max_lat_us` | `total_ns`/`count`、`max_ns` | 单次耗时 |
| `total_ms_per_sec` | `total_ns` 差分 | 累计耗时占比 |

根因判定：单次平均耗时 ≥1ms → "高耗时热点(阻塞型，如 fsync)"；否则 → "高频热点(忙轮询/未批处理)"。
触发条件：频次 ≥ 阈值 **或** 累计耗时 ≥ 300ms/秒。
> 注意：raw_syscalls 触发极频繁，是开销最高的场景；扩展点见下，可加 target_pid 过滤只观测目标进程。
