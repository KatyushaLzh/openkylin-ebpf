...
仅 major fault 升高 → "工作集超出物理内存"；没有 direct reclaim 时退到最大 RSS 进程；
...
用户态按架构(amd64 专用表 / asm-generic 表)解析。可通过 `--target-pid` 只观测指定 tgid，
降低 `raw_syscalls` 全局挂载带来的开销。
...
| `target_pid` | CLI 配置 | 进程过滤目标；0 表示全局 |
